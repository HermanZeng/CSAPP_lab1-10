# 5140219405 ZengHeming
# I add the IADDL and LEAVE insruction. To implement jump table in 
# y86 ISA, I use mrmovl, pushl and ret insructions to simulate it.
# To solve Load/Use hazard in pushl, ret, so I forward E_valA of pushl
# when ret instruction in Fetch stage. Another Load/Use hazard will 
# occur in mrmovl, pushl, so I insert a bubble into it.
# Futhermore, when jle prediction fails, it will cost high penalty, so
# I changed ifun. As we know that when jle comes in, the Cnd will be 
# calculated in Execute stage using CC, so it may cause 3 bubbles, so 
# I modified some instructions such as andl to calculate Cnd immediately
# so that jle can use it directly to avoid wrong prediction.
# 
#/* $begin pipe-all-hcl */
####################################################################
...
bool F_stall =
	...
	E_icode in { IMRMOVL, IPOPL, ILEAVE } && ((E_dstM == d_srcA && !(D_icode in {IRMMOVL, IOPL}) && !(D_icode == IPUSHL && f_icode != IRET)) || (E_dstM == d_srcB && D_icode != IOPL)) ||
	# Stalling at fetch while ret passes through pipeline
    ((D_icode == IRET && E_icode != IPUSHL) || (E_icode == IRET && M_icode != IPUSHL) || (M_icode == IRET && W_icode != IPUSHL));
...
bool D_bubble =
	# Mispredicted branch
    (D_icode == IJXX && !e_Cnd && !(E_icode in { IOPL, IIADDL }) && D_ifun == E_ifun && !(!(M_icode in { IOPL, IIADDL }) && D_ifun == M_ifun)) || (E_icode == IJXX && !e_Cnd && !(!(M_icode in { IOPL, IIADDL }) && E_ifun == M_ifun)
...
bool E_bubble =
	# Mispredicted branch
    (E_icode == IJXX && !e_Cnd && !(!(M_icode in { IOPL, IIADDL}) && !(W_icode in { IOPL, IIADDL, IJXX, IRRMOVL }) && E_ifun == 1)) ||
	# Conditions for a load/use hazard
    # but not condition for load forwarding
	E_icode in { IMRMOVL, IPOPL, ILEAVE } && ((E_dstM == d_srcA && !(D_icode in {IRMMOVL, IPUSHL, IOPL})) || (E_dstM == d_srcB && D_icode != IOPL));
