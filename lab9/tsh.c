...
void eval(char *cmdline) 
{
    ...
    if (!builtin_cmd(argv))
    {
        ...
        if (sigprocmask(SIG_BLOCK, &mask, NULL) < 0) perror("sigprocmask");  /* Block SIGCHLD */
...
void sigchld_handler(int sig) 
{
    pid_t pid; /* Process recycled by parent */
    int status; /* Process status */
    int jid; /* Job id */
    
    while((pid = waitpid(-1, &status, WNOHANG|WUNTRACED)) > 0) /* Recycle terminated child process */
    {
...
void sigint_handler(int sig) 
{
    pid_t fg = fgpid(jobs); /* Foreground process */
    if(kill(-fg, SIGINT) < 0)
...
void sigtstp_handler(int sig) 
{
    ...
    if(kill(-fg, SIGSTOP) < 0)
        perror("kill in sigtstp_handler");
    return;
}
