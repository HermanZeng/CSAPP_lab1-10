...
{    
	...
	symbol_t *temp_symbol=(symbol_t*) malloc (sizeof(symbol_t));
    temp_symbol->name=name;
    temp_symbol->addr=vmaddr; 
    temp_symbol->next=symtab->next;

    /* add the new symbol_t to symbol table */
	symtab->next=temp_symbol;
    return 0;
}
...
parse_t parse_instr(char **ptr, instr_t **inst)
{
    ...
    tmp = find_instr(current);
    if (tmp == NULL)
        return PARSE_ERR;

    current += tmp->len;
    if (!IS_END(current) && !IS_BLANK(current))
        return PARSE_ERR;
...
parse_t parse_delim(char **ptr, char delim)
{
	...
	if (*current != delim){
		if (IS_REG(current+1) || IS_LETTER(current+1) || IS_DIGIT(current+1))
		err_print("Invalid ','");
		return PARSE_ERR;
	}	
	current ++;
...
parse_t parse_symbol(char **ptr, char **name)
{
	...
	int k = 0;
	char *temp_name;
	while (IS_LETTER(current + k) || IS_DIGIT(current + k)) {
		k++;
		if (k == strlen(current))
			break;
	}
	temp_name = (char *)malloc(k*sizeof(char)+1);
	strncpy(temp_name,current,k);
    temp_name[k+1]='\0';
...
parse_t parse_imm(char **ptr, char **name, long *value)
{
	...
	if (IS_IMM(current)){
		current++;
		result = parse_digit(&(current),temp_val);
	}

    /* if IS_LETTER, then parse the symbol */
    else if (IS_LETTER(current))
		result = parse_symbol(&(current),&(temp_name));
...
parse_t parse_mem(char **ptr, long *value, regid_t *regid)
{
    ...
    /* skip the blank and check */
	SKIP_BLANK(current);
    if (IS_END(current)) return PARSE_ERR;
        

    /* calculate the digit and register, (ex: (%ebp) or 8(%ebp)) */
	if (parse_digit(&current,&temp_val) == PARSE_ERR)
        temp_val = 0;
	if (*(current) == '('){
		current++;
		if(parse_reg(&(current),&temp_reg) == PARSE_ERR)
            return PARSE_ERR;
		if (*(current) != ')'){
			current++;	
...
parse_t parse_data(char **ptr, char **name, long *value)
{
	...
	if (IS_DIGIT(current))
		result = parse_digit(&current,value);

    /* if IS_LETTER, then parse the symbol */
	if (IS_LETTER(current))
		result = parse_symbol(&current,name);
...
parse_t parse_label(char **ptr, char **name)
{
    ...
    SKIP_BLANK(current);
    SKIP_BLANK(*ptr);
    if (IS_END(current)||!IS_LETTER(current)) return PARSE_ERR;
    /* allocate name and copy to it */
...
while(1) {
    ...
    SKIP_BLANK(y86asm);
    if (IS_END(y86asm)) break;
        
    /* is a comment ? */
    if (IS_COMMENT(y86asm)) break;
        
    /* is a label ? */
    char *label = NULL;    
    if (parse_label(&y86asm, &label) == PARSE_LABEL) {
    ...
    instr_t *inst = NULL;
    /* is an instruction ? */
    if (parse_instr(&y86asm, &inst) == PARSE_ERR) {
        line->type = TYPE_ERR;
    ...
    switch (HIGH(inst->code)) {
      ...
      case I_POPL: {
		regid_t reg_buff;
		if (parse_reg(&y86asm,&reg_buff) == PARSE_ERR){
			line->type = TYPE_ERR;
      ...
      case I_ALU: { 
		regid_t regA,regB;
		char de = ',';
		if (parse_reg(&y86asm,&regA) == PARSE_ERR){
			line->type = TYPE_ERR;
			err_print("Invalid REG");
            END_LOOP = TRUE;
		}
		if (parse_delim(&y86asm,de) == PARSE_ERR){
			line->type = TYPE_ERR;
            END_LOOP = TRUE;
		}
		if (parse_reg(&y86asm,&regB) == PARSE_ERR){
			line->type = TYPE_ERR;
			err_print("Invalid REG");
            END_LOOP = TRUE;
		}
      ...
      case I_IRMOVL: {  
		...
		char *name;
		long val = 10;
		parse_t result = parse_imm(&y86asm,&name,&val);
		if (result == PARSE_ERR){
			line->type = TYPE_ERR;
            END_LOOP = TRUE;
            break;
      ...
      case I_RMMOVL: {  
		...
		if (parse_mem(&y86asm,&val,&regB) == PARSE_ERR){
			line->type = TYPE_ERR;
            END_LOOP = TRUE;
            break;
		}
      ...
      case I_CALL: {
		...
		char *name;
		parse_t result = PARSE_ERR; 
		result = parse_data(&y86asm,&name,&val);
      ...
      case I_DIRECTIVE: {
        switch (LOW(inst->code)) {
          case D_DATA: {   
			long val = 0;
			char *name;
			parse_t result = parse_data(&y86asm,&name,&val);
			if (result == PARSE_SYMBOL)
				add_reloc(name,y86bin);
			if (result == PARSE_DIGIT){
          ...
          case D_POS: {  
			long val = 0;
			if (parse_digit(&y86asm,&val) == PARSE_ERR)
				err_print("Parse position error");
			vmaddr = val;
			y86bin->addr = vmaddr;
            break;
          }
          ...
          case D_ALIGN: {   
			long val = 0;
			if (parse_digit(&y86asm,&val) == PARSE_ERR)
				err_print("Parse alignment error");
            long remainer = vmaddr % val;
			if (remainer)
				vmaddr = vmaddr + val - remainer;
			y86bin->addr = vmaddr;
			break;
          }
          default:
...
int binfile(FILE *out)
{
    ...
    {	//printf("%d %d\n", p->y86bin.addr, HIGH(p->y86bin.codes[0]));
	    ...
	    while(temp)
	    {
	        if(temp->y86bin.bytes != 0) {END = TRUE; break;}
	            temp = temp->next;
	    }
	    //return 1;
    
        if(pos < p->y86bin.addr && END) //fill blanks
	    {
